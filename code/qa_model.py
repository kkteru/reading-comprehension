...
from modules import RNNEncoder, SimpleSoftmaxLayer, Attn
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob, 'RNNEncoder1')
        print 'Context embedding shape : ', self.context_embs.get_shape()
        context_hiddens = encoder.build_graph(self.context_embs, self.context_mask)  # (batch_size, context_len, hidden_size*2)
        question_hiddens = encoder.build_graph(self.qn_embs, self.qn_mask)  # (batch_size, question_len, hidden_size*2)

        # Use context hidden states to attend to question hidden states
        attn_layer = Attn(self.keep_prob, self.FLAGS.hidden_size * 2, self.FLAGS.hidden_size * 2)
        ...
        # print blended_reps_condensed.get_shape()
        # blended_reps_encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob, 'RNNEncoder2')
    ...
    def get_start_end_pos(self, session, batch):
        ...
        if tf.app.flags.FLAGS.eval_squad_2:
            nextMax.append(np.concatenate((start_dist[:, -1][:, np.newaxis], self.FLAGS.context_len * np.ones((tf.shape(start_dist).eval()[0], 1))), axis=1))
        ...
        while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
            ...
            for batch in get_batch_generator(self.word2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, discard_long=True):
                ...
                if global_step % self.FLAGS.eval_every == 0:
                    ...
                    # Early stopping based on dev loss. You could switch this to use F1 instead.
